...
class SidePanel(Window):
        ...
        self.add(
            Box(
                ...
                children=[
                    ...
                    Label(
                        label=f"Good {'Morning' if time.localtime().tm_hour < 12 else 'Afternoon'}, {os.getlogin().capitalize()}",
    ...
    def get_current_uptime(self):
        ...
        return f"{int(uptime_days)} {'days' if uptime_days != 1 else 'day'}, {int(uptime_hours)} {'hours' if uptime_hours != 1 else 'hour'}, {int(uptime_minutes)} {'minutes' if uptime_minutes != 1 else 'minute'} "
